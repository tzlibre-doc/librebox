...
export default function () {
  ...
  r.loadSetting = function () {
    ...
    if (!settings.version || settings.version < LOCAL_STORAGE_VERSION) {
      settings = {
        ...settings,
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        try {
          ...
          const sk = sjcl.decrypt(window.eztzl.library.pbkdf2.pbkdf2Sync(password, ss.pkh, 30000, 512, 'sha512').toString(), ss.ensk)
          if (sk.substr(0, 4) === 'edsk') {
            keys = window.eztzl.crypto.extractKeys(sk)
          } else {
            keys = {
