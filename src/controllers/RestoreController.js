...
export default ['$scope', '$location', 'Storage', 'SweetAlert', function ($scope, $location, Storage, SweetAlert) {
  ...
  $scope.restore = function () {
    ...
    if (['seed', 'ico'].indexOf($scope.type) >= 0 && !window.eztzl.library.bip39.validateMnemonic($scope.seed)) return SweetAlert.swal('Uh-oh!', "Your seed words are not valid - please check to ensure you are not missing a word/letter, and you haven't included an extra space/line break")
    ...
    if ($scope.type === 'seed') {
      keys = window.eztzl.crypto.generateKeys($scope.seed, $scope.passphrase)
    } else if ($scope.type === 'ico') {
      keys = window.eztzl.crypto.generateKeys($scope.seed, $scope.email + $scope.ico_password)
      ...
      keys = window.eztzl.crypto.extractKeys($scope.private_key)
    }
    ...
    if ($scope.type === 'ico' && $scope.activation_code) {
      popup.showLoader()
      window.eztzl.rpc.activate(keys.pkh, $scope.activation_code).then(function () {
