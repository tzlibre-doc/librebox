    ...
    return {
      send: async (from, to, amount, fee, parameters, type, gasLimit = config.txGasLimit, storageLimit = config.txStorageLimit) => {
        ...
        try {
          ...
          if (parameters) {
            tx = await window.eztzl.contract.send(to, from, keysWithourSkIfLedger, amount, parameters, fee, gasLimit, storageLimit)
          } else {
            tx = await window.eztzl.rpc.transfer(from, keysWithourSkIfLedger, to, amount, fee, parameters, gasLimit, storageLimit)
          }
          if (type === 'ledger') {
            const signedTxByLedger = await window.tezledger.sign(keys.sk, '03' + tx.opbytes)
            tx.opOb.signature = window.eztzl.utility.b58cencode(window.eztzl.utility.hex2buf(signedTxByLedger.signature), window.eztzl.prefix.edsig)
            await window.eztzl.rpc.inject(tx.opOb, tx.opbytes + signedTxByLedger.signature)
          }
          SweetAlert.swal('Awesome!', 'Transaction has been sent - this may take a few minutes to be included on the blockchain', 'success')
        } catch (e) {
          if (e.message !== 'Cancel') {
            SweetAlert.swal('Uh-oh!', e.message || 'Operation Failed! Please check your input or retry later.')
          }
        }
        popup.hideLoader()
      },
      delegate: async (type, address, addressToDelegate) => {
        try {
          ...
          const keysWithourSkIfLedger = Object.assign({}, keys, { sk: type === 'ledger' ? false : keys.sk })
          const tx = await window.eztzl.rpc.setDelegate(address, keysWithourSkIfLedger, addressToDelegate, 1257)

          if (type === 'ledger') {
            const rr = await window.tezledger.sign(keys.sk, '03' + tx.opbytes)
            tx.opOb.signature = window.eztzl.utility.b58cencode(window.eztzl.utility.hex2buf(rr.signature), window.eztzl.prefix.edsig)
            const result = await window.eztzl.rpc.inject(tx.opOb, tx.opbytes + rr.signature)
          }
      ...
      originOnTzl: async (type) => {
        try {
          ...
          const keys = await Storage.decryptPrivateKeys(password)
          const keysWithourSkIfLedger = Object.assign({}, keys, { sk: type === 'ledger' ? false : keys.sk })
          let tx = await window.eztzl.rpc.account(keysWithourSkIfLedger, amount, true, true, null,
            config.originFee, config.originGasLimit, config.originStorageLimit)
          if (type === 'ledger') {
            const rr = await window.tezledger.sign(keys.sk, '03' + tx.opbytes)
            tx.opOb.signature = window.eztzl.utility.b58cencode(window.eztzl.utility.hex2buf(rr.signature), window.eztzl.prefix.edsig)
            tx = await window.eztzl.rpc.inject(tx.opOb, tx.opbytes + rr.signature)
          }
          ...
          return window.eztzl.contract.hash(tx.hash, 0)
        } catch (e) {
          const msgError = e.message || 'Operation Failed! Please check your input or retry later.'
      ...
      signEthAddress: async (type, ethAddress = undefined, title, description) => {
        ...
        try {
          ...
          if (type === 'ledger') {
            ...
            proof = await window.eztzl.crypto.sign(ethAddress, keys.sk).sig
            proof = window.eztzl.utility.buf2hex(proof)
          }
          popup.hideLoader()
          let tzlPk = window.eztzl.utility.b58cdecode(keys.pk, window.eztzl.prefix.edpk)
          tzlPk = window.eztzl.utility.buf2hex(tzlPk)
      ...
      signMessage: async (type, message, title, description) => {
        ...
        try {
          ...
          if (type === 'ledger') {
            ...
            proof = await window.eztzl.crypto.sign(message, keys.sk).sig
            proof = window.eztzl.utility.buf2hex(proof)
          }
          popup.hideLoader()
          let tzlPk = window.eztzl.utility.b58cdecode(keys.pk, window.eztzl.prefix.edpk)
